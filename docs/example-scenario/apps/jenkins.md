...
- **Jenkins-Server:** Ein virtueller Computer, der als Jenkins-Master fungiert, wird bereitgestellt, um [Jenkins][azure-market] als Automatisierungsserver auszuführen. Verwenden Sie zum Bereitstellen dieser VM die [Lösungsvorlage für Jenkins auf Azure][solution]. Andere Jenkins-Angebote sind im Azure Marketplace erhältlich.
...
Es wird empfohlen, sich das folgende [Azure-Beispielszenario](/azure/architecture/example-scenario) anzusehen, das veranschaulicht, wie bestimmte Lösungen einige derselben Technologien verwenden:

- [CI/CD-Pipeline für containerbasierte Workloads](/azure/architecture/example-scenario/apps/devops-with-aks)
