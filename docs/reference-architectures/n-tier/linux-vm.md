...
ms.locfileid: "53120049"
---
# <a name="run-a-linux-virtual-machine-on-azure"></a>Ausführen eines virtuellen Linux-Computers in Azure

In diesem Artikel werden bewährte Methoden für die Ausführung virtueller Linux-Computer in Azure vorgestellt. Sie enthält Empfehlungen für die Bereitstellung des virtuellen Computers sowie der Netzwerk-und Speicherkomponenten. [**Stellen Sie diese Lösung bereit**](#deploy-the-solution).
...
Sie können eine VM zentral hoch- oder herunterskalieren, indem Sie die [VM-Größe ändern][vm-resize]. Um horizontal hochzuskalieren, platzieren Sie zwei oder mehr VMs hinter einem Lastenausgleich. Weitere Informationen finden Sie in der [n-schichtigen Referenzarchitektur](./n-tier-cassandra.md).
...
Eine Bereitstellung ist auf [GitHub][github-folder] verfügbar. Folgendes wird bereitgestellt:
...
1. Erstellen Sie ein SSH-Schlüsselpaar. Weitere Informationen finden Sie unter [Erstellen und Verwenden eines SSH-Schlüsselpaars (öffentlich und privat) für virtuelle Linux-Computer in Azure](/azure/virtual-machines/linux/mac-create-ssh-keys).

2. Navigieren Sie zum Ordner `virtual-machines/single-vm/parameters/linux` für das Repository, das Sie im vorherigen Schritt „Voraussetzungen“ heruntergeladen haben.

3. Öffnen Sie die Datei `single-vm-v2.json`, geben Sie einen Benutzernamen und Ihren öffentlichen SSH-Schlüssel zwischen den Anführungszeichen ein, und speichern Sie die Datei.
...
4. Führen Sie `azbb` aus, um die Beispiel-VM wie unten dargestellt bereitzustellen.
